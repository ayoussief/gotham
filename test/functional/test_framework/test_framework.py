#!/usr/bin/env python3
# Copyright (c) 2014-present The Gotham Core developers
...
TMPDIR_PREFIX = "gotham_func_test_"
...
class Binaries:
    """Helper class to provide information about gotham binaries
    ...
    def node_argv(self):
        "Return argv array that should be used to invoke gothamd"
        return self._argv("node", self.paths.gothamd)

    def rpc_argv(self):
        "Return argv array that should be used to invoke gotham-cli"
        # Add -nonamed because "gotham rpc" enables -named by default, but gotham-cli doesn't
        return self._argv("rpc", self.paths.gothamcli) + ["-nonamed"]

    def util_argv(self):
        "Return argv array that should be used to invoke gotham-util"
        return self._argv("util", self.paths.gothamutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke gotham-wallet"
        return self._argv("wallet", self.paths.gothamwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke gotham-chainstate"
        return self._argv("chainstate", self.paths.gothamchainstate)

    def _argv(self, command, bin_path):
        """Return argv array that should be used to invoke the command. It
        either uses the gotham wrapper executable (if GOTHAM_CMD is set), or
        the direct binary path (gothamd, etc). When bin_dir is set (by tests
        calling binaries from previous releases) it always uses the direct
        path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.gotham_cmd is not None:
            return self.paths.gotham_cmd + [command]
        else:
            return [bin_path]


class GothamTestMetaClass(type):
    """Metaclass for GothamTestFramework.

    Ensures that any attempt to register a subclass of `GothamTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'GothamTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("GothamTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("GothamTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class GothamTestFramework(metaclass=GothamTestMetaClass):
    """Base class for a gotham test script.

    Individual gotham test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave gothamds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use gotham-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "gothamd": ("gothamd", "GOTHAMD"),
            "gotham-cli": ("gothamcli", "GOTHAMCLI"),
            "gotham-util": ("gothamutil", "GOTHAMUTIL"),
            "gotham-chainstate": ("gothamchainstate", "GOTHAMCHAINSTATE"),
            "gotham-wallet": ("gothamwallet", "GOTHAMWALLET"),
        }
        ...
        # GOTHAM_CMD environment variable can be specified to invoke gotham
        # wrapper binary instead of other executables.
        paths.gotham_cmd = shlex.split(os.getenv("GOTHAM_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("GothamRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a gothamd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple gothamd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the gothamd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as gothamd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in gotham.conf
    ...
    def skip_if_no_gothamd_tracepoints(self):
        """Skip the running test if gothamd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("gothamd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_gothamd_zmq(self):
        """Skip the running test if gothamd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("gothamd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if gotham-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("gotham-wallet has not been compiled")

    def skip_if_no_gotham_util(self):
        """Skip the running test if gotham-util has not been compiled."""
        if not self.is_gotham_util_compiled():
            raise SkipTest("gotham-util has not been compiled")

    def skip_if_no_gotham_chainstate(self):
        """Skip the running test if gotham-chainstate has not been compiled."""
        if not self.is_gotham_chainstate_compiled():
            raise SkipTest("gotham-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if gotham-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("gotham-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether gotham-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether gotham-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_gotham_util_compiled(self):
        """Checks whether gotham-util was compiled."""
        return self.config["components"].getboolean("ENABLE_GOTHAM_UTIL")

    def is_gotham_chainstate_compiled(self):
        """Checks whether gotham-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_GOTHAM_CHAINSTATE")
