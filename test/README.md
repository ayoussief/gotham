This directory contains integration tests that test gothamd and its
...
gothamd and gotham-qt by interacting with them through the RPC and P2P
interfaces.
- [util](/test/util) which tests the utilities (gotham-util, gotham-tx, ...).
...
Before tests can be run locally, Gotham Core must be built.  See the [building instructions](/doc#building) for help.
...
The P2P and RPC ports used by the gothamd nodes-under-test are chosen to make
conflicts with other processes unlikely. However, if there is another gothamd
process running on the system (perhaps from a previous test which hasn't successfully
killed all its gothamd nodes), then there may be a port conflict which will
cause the test to fail. It is recommended that you run the tests on a system
where no other gothamd processes are running.

On linux, the test framework will warn if there is another
gothamd process running when the tests are started.

If there are zombie gothamd processes after test failure, you can kill them
by running the following commands. **Note that these commands will kill all
gothamd processes running on the system, so should not be used if any non-test
gothamd processes are being run.**
...
sure gothamd processes are stopped as above):
...
- when run by [our CI (Continuous Integration)](/ci/README.md), no logs are output to the console. However, if a test
  fails, the `test_framework.log` and gothamd `debug.log`s will all be dumped
...
`test_framework.log` and gothamd `debug.log`s can be combined into a single
...
call methods that interact with the gothamd nodes-under-test.

If further introspection of the gothamd instances themselves becomes
...
```bash
cat /tmp/user/1000/testo9vsdjo3/node1/regtest/gothamd.pid
```

Then you can use the pid to start `gdb`:

```bash
gdb /home/example/gothamd <pid>
```
