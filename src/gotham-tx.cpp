...
static void SetupGothamTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " gotham-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The gotham-tx tool is used for creating and modifying gotham transactions.\n\n"
                "gotham-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded gotham transaction, or with \"-create [commands]\" to create a hex-encoded gotham transaction.\n"
                "\n"
                "Usage: gotham-tx [options] <hex-tx> [commands]\n"
                "or:    gotham-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded gotham transaction
