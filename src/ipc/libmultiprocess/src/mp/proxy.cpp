...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking gotham code so they have to run
    ...
    // functions run, so client objects are fully disconnected before gotham
    // code in the destructors are run. This way if the gotham code tries to
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In gotham core, running these callbacks in the right order is
