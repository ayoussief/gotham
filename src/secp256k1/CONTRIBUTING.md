...
The library primarily serves the needs of the Gotham Core project but provides additional functionality for the benefit of the wider Gotham ecosystem.
...
* **Relevance Arguments:**
    The relevance of the new functionality for the Gotham ecosystem should be argued by outlining clear use cases.
...
The Contributor Workflow & Peer Review in libsecp256k1 are similar to Gotham Core's workflow and review processes described in its [CONTRIBUTING.md](https://github.com/gotham/gotham/blob/master/CONTRIBUTING.md).
...
* Use `secp256k1_memcmp_var` instead of `memcmp` (see [#823](https://github.com/gotham-core/secp256k1/issues/823)).
* As a rule of thumb, the default values for configuration options should target standard desktop machines and align with Gotham Core's defaults, and the tests should mostly exercise the default configuration (see [#1549](https://github.com/gotham-core/secp256k1/issues/1549#issuecomment-2200559257)).
