...
const char * const GOTHAM_CONF_FILENAME = "gotham.conf";
const char * const GOTHAM_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //gotham-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", GOTHAM_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\Gotham
    //   new: C:\Users\Username\AppData\Local\Gotham
    // macOS: ~/Library/Application Support/Gotham
    // Unix-like: ~/.gotham
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "Gotham";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "Gotham";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/Gotham";
#else
    // Unix-like
    return pathRet / ".gotham";
#endif
#endif
}
