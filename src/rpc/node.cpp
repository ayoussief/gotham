...
static RPCHelpMan echo(const std::string& name)
{
    return RPCHelpMan{
        ...
        "Simply echo back the input arguments. This command is for testing.\n"
                ...
                "gotham-cli and the GUI. There is no server-side difference.",
        {
    ...
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue {
            ...
            if (interfaces::Ipc* ipc = local_init.ipc()) {
                // Spawn a new gotham-node process and call makeEcho to get a
                // client pointer to a interfaces::Echo instance running in
                // that process. This is just for testing. A slightly more
                // realistic test spawning a different executable instead of
                // the same executable would add a new gotham-echo executable,
                // and spawn gotham-echo below instead of gotham-node. But
                // using gotham-node avoids the need to build and install a
                // new executable just for this one test.
                auto init = ipc->spawnProcess("gotham-node");
                echo = init->makeEcho();
                ipc->addCleanup(*echo, [init = init.release()] { delete init; });
            } else {
                // IPC support is not available because this is a gothamd
                // process not a gothamd-node process, so just create a local
