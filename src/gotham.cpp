...
Options:
  -m, --multiprocess     Run multiprocess binaries gotham-node, gotham-gui.
  -M, --monolithic       Run monolithic binaries gothamd, gotham-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show this help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'gotham-qt [ARGS]' or 'gotham-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'gothamd [ARGS]' or 'gotham-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'gotham-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'gotham-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'gotham-tx [ARGS]'.
...
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench_gotham [ARGS]'.
  chainstate [ARGS] Run gotham kernel chainstate util, equivalent to running 'gotham-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_gotham [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_gotham-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(cmd.use_multiprocess ? "gotham-gui" : "gotham-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(cmd.use_multiprocess ? "gotham-node" : "gothamd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("gotham-cli");
            // Since "gotham rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("gotham-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("gotham-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench_gotham");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("gotham-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_gotham");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_gotham-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("gotham-util");
        } else {
...
//! Execute the specified gothamd, gotham-qt or other command line in `args`
...
//!             be a relative path to a gothamd, gotham-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin/gotham ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com/gotham/gotham/pull/31375#discussion_r1861814807)
