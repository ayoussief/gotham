...
static bool ParseArgs(NodeContext& node, int argc, char* argv[])
{
    ...
    // If Qt is used, parameters/gotham.conf are parsed in qt/gotham.cpp's main()
    ...
    for (int i = 1; i < argc; i++) {
        if (!IsSwitchChar(argv[i][0])) {
            return InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see gothamd -h for a list of options.", argv[i])));
        }
    }
    return true;
}
...
static bool ProcessInitCommands(ArgsManager& args)
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        if (args.GetBoolArg("-version", false)) {
            ...
            strUsage += "\n"
                "The " CLIENT_NAME " daemon (gothamd) is a headless program that connects to the Gotham network to validate and relay transactions and blocks, as well as relaying addresses.\n\n"
                "It provides the backbone of the Gotham network and its RPC, REST and ZMQ services can provide various transaction, block and address-related services.\n\n"
...
static bool AppInit(NodeContext& node)
{
    ...
    try
    {
        // -server defaults to true for gothamd but not for the GUI so do this here
...
MAIN_FUNCTION
{
    ...
    // Connect gothamd signal handlers
    noui_connect();
