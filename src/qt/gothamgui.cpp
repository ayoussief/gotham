...
const std::string GothamGUI::DEFAULT_UIPLATFORM =
...
GothamGUI::GothamGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &GothamGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &GothamGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &GothamGUI::showModalOverlay);
...
void GothamGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a Gotham address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and gotham: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &GothamGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &GothamGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &GothamGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your Gotham addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified Gotham addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed Gotham Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed Gotham Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a gotham: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible Gotham command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &GothamGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &GothamGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &GothamGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &GothamGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &GothamGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &GothamGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &GothamGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &GothamGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &GothamGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &GothamGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &GothamGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &GothamGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &GothamGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &GothamGUI::showDebugWindow);
}

void GothamGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void GothamGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &GothamGUI::setCurrentWalletBySelectorIndex);
...
void GothamGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &GothamGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &GothamGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &GothamGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &GothamGUI::showProgress);
...
#ifdef ENABLE_WALLET
void GothamGUI::enableHistoryAction(bool privacy)
{
...
void GothamGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &GothamGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &GothamGUI::removeWallet);
...
WalletController* GothamGUI::getWalletController()
{
    return m_wallet_controller;
}

void GothamGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &GothamGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &GothamGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &GothamGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &GothamGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &GothamGUI::incomingTransaction);
    connect(this, &GothamGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void GothamGUI::removeWallet(WalletModel* walletModel)
{
...
void GothamGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void GothamGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void GothamGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void GothamGUI::setWalletActionsEnabled(bool enabled)
{
...
void GothamGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &GothamGUI::toggleHidden);
...
void GothamGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void GothamGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void GothamGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void GothamGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void GothamGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void GothamGUI::openClicked()
{
...
void GothamGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void GothamGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void GothamGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void GothamGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void GothamGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void GothamGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void GothamGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void GothamGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to Gotham network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void GothamGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void GothamGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void GothamGUI::updateHeadersSyncProgressLabel()
{
...
void GothamGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void GothamGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &GothamGUI::quitRequested);
...
void GothamGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void GothamGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &GothamGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void GothamGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void GothamGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &GothamGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &GothamGUI::show);
                e->ignore();
            }
        }
    }
...
void GothamGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void GothamGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void GothamGUI::incomingTransaction(const QString& date, GothamUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(GothamUnits::formatWithUnit(unit, amount, true));
...
void GothamGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void GothamGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool GothamGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool GothamGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void GothamGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void GothamGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void GothamGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void GothamGUI::updateProxyIcon()
{
    std::string ip_port;
...
void GothamGUI::updateWindowTitle()
{
...
void GothamGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void GothamGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void GothamGUI::detectShutdown()
{
...
void GothamGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void GothamGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(GothamGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = GothamGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void GothamGUI::subscribeToCoreSignals()
{
...
void GothamGUI::unsubscribeFromCoreSignals()
{
...
bool GothamGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<GothamUnit> units = GothamUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const GothamUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, GothamUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const GothamUnit u : GothamUnits::availableUnits()) {
        menu->addAction(GothamUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(GothamUnit newUnits)
{
    setText(GothamUnits::longName(newUnits));
}
