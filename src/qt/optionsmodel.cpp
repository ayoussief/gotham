...
/** Call node.updateRwSetting() with Gotham 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Gotham 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/gotham/gotham/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because gotham 22.x will not try to read these.
...
//! Convert enabled/size values to gotham -prune setting.
...
//! Get pruning enabled value to show in GUI from gotham -prune setting.
...
//! Get pruning size value to show in GUI from gotham -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayGothamUnit")) {
        settings.setValue("DisplayGothamUnit", QVariant::fromValue(GothamUnit::GTH));
    }
    QVariant unit = settings.value("DisplayGothamUnit");
    if (unit.canConvert<GothamUnit>()) {
        m_display_gotham_unit = unit.value<GothamUnit>();
    } else {
        m_display_gotham_unit = GothamUnit::GTH;
        settings.setValue("DisplayGothamUnit", QVariant::fromValue(m_display_gotham_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if gotham.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_gotham_unit);
...
{
    if (new_unit.isNull() || new_unit.value<GothamUnit>() == m_display_gotham_unit) return;
    m_display_gotham_unit = new_unit.value<GothamUnit>();
    QSettings settings;
    settings.setValue("DisplayGothamUnit", QVariant::fromValue(m_display_gotham_unit));
    Q_EMIT displayUnitChanged(m_display_gotham_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/gotham/gotham/pull/8273
    ...
    // (https://github.com/gotham-core/gui/issues/567).
