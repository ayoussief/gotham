// Copyright (c) 2015-2022 The Gotham Core developers
                  ...
                  QMessageBox::StandardButton confirm_type = QMessageBox::Yes)
{
    ...
    entry->findChild<GothamAmountField*>("payAmount")->setValue(amount);
...
void CompareBalance(WalletModel& walletModel, CAmount expected_balance, QLabel* balance_label_to_check)
{
    ...
    QString balanceComparison = GothamUnits::formatWithUnit(unit, expected_balance, false, GothamUnits::SeparatorStyle::ALWAYS);
...
//
//     QT_QPA_PLATFORM=xcb     build/bin/test_gotham-qt  # Linux
//     QT_QPA_PLATFORM=windows build/bin/test_gotham-qt  # Windows
//     QT_QPA_PLATFORM=cocoa   build/bin/test_gotham-qt  # macOS
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    // Amount input
    GothamAmountField* amountInput = receiveCoinsDialog.findChild<GothamAmountField*>("reqAmount");
...
void WalletTests::walletTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        qWarning() << "Skipping WalletTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
                      "with 'QT_QPA_PLATFORM=cocoa test_gotham-qt' on mac, or else use a linux or windows build.";
        return;
    }
#endif
    TestGUI(m_node);
