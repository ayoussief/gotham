...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<GothamUnit>("GothamUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. gotham_de.qm (shortcut "de" needs to be defined in gotham.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. gotham_de_DE.qm (shortcut "de_DE" needs to be defined in gotham.qrc)
...
static const char* qt_argv = "gotham-qt";

GothamApplication::GothamApplication()
...
void GothamApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the GothamApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", GothamGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void GothamApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool GothamApplication::createOptionsModel(bool resetSettings)
{
...
void GothamApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new GothamGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &GothamGUI::quitRequested, this, &GothamApplication::requestShutdown);
...
void GothamApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void GothamApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool GothamApplication::baseInitialize()
{
    return node().baseInitialize();
}

void GothamApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &GothamApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &GothamApplication::handleRunawayException);
    connect(this, &GothamApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &GothamApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void GothamApplication::parameterSetup()
{
...
void GothamApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void GothamApplication::requestInitialize()
{
...
void GothamApplication::requestShutdown()
{
...
void GothamApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // gotham: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GothamGUI::handlePaymentRequest);
            connect(window, &GothamGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void GothamApplication::handleRunawayException(const QString &message)
{
...
void GothamApplication::handleNonFatalException(const QString& message)
{
...
WId GothamApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool GothamApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", GothamGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(gotham);
    Q_INIT_RESOURCE(gotham_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(GOTHAM_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see gotham-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see gotham-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse gotham.conf, determine network, switch to network specific
    ...
    // gotham: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
