...
Below are some notes on how to build Gotham Core for Windows.

The options known to work for building Gotham Core on Windows are:
...
Note that for WSL the Gotham Core source path MUST be somewhere in the default mount file system, for
example /usr/src/gotham, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\gotham`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/gotham
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/gotham --strip
```
