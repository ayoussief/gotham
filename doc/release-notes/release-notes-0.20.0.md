...
Gotham Core version 0.20.0 is now available from:

  <https://gothamcore.org/bin/gotham-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from Gotham Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since Gotham Core version 0.18.0.
...
- Testing or debugging of implementations of the Gotham P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of Gotham Core. Gotham Core logs debug messages
...
- OpenSSL is no longer used by Gotham Core.  (#17265)

- BIP70 support has been fully removed from Gotham Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/gotham/gotham/issues/16599),
  [PR #16702](https://github.com/gotham/gotham/pull/16702), and the
  `gothamd help` for more information.  This option is experimental and
...
- All custom settings configured when Gotham Core starts are now
...
- The "Start Gotham Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://gothamcore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple Gotham Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed Gotham Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/gotham-core/HWI) for
  signing.  Future versions of Gotham Core should support a GUI option
...
- Gotham Core's automatically-generated source code documentation is
  now available at https://doxygen.gothamcore.org.  (#17596)
...
- The `gotham-cli` utility used with the `-getinfo` parameter now
...
- The `gotham-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `gothamstrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_gotham-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBGOTHAM_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_gotham` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBGOTHAM_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for gotham-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate gotham schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest gotham.conf (sdaftuar)
...
- #18022 Fix appveyor `test_gotham` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (gotham-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone gotham-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about Gotham Core files (hebasto)
...
- #17119 Fix broken gotham-cli examples (andrewtoth)
...
- #17503 Remove bitness from gotham-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.gothamcore.org (laanwj)
...
- #17648 Rename wallet-tool references to gotham-wallet (hel-o)
...
- #18341 Replace remaining literal GTH with `CURRENCY_UNIT` (domob1812)
...
- #16115 On gothamd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from gotham-cli -getinfo (malevolent)
