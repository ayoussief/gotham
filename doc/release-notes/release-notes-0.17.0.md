Gotham Core version 0.17.0 is now available from:

  <https://gothamcore.org/bin/gotham-core-0.17.0/>
...
Gotham Core is extensively tested on multiple operating systems using
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `gotham.conf` file, not inside included files or from
  ...
  as gotham.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the gotham.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=gotham
    test.uacomment=gotham-testnet
...
be used in V0.17 by starting gothamd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Gotham Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/gotham/bips/blob/master/bip-0174.mediawiki) is an interchange format for Gotham transactions that are not fully signed
yet, together with relevant metadata to help entities work towards signing it.
It is intended to simplify workflows where multiple parties need to cooperate to
produce a transaction. Examples include hardware wallets, multisig setups, and
[CoinJoin](https://gothamtalk.org/?topic=279249) transactions.

### Overall workflow

Overall, the construction of a fully signed Gotham transaction goes through the
following steps:
...
- An **Extractor** produces a valid Gotham transaction (in network format)
...
- New `fees` field introduced in `getrawmempool`, `getmempoolancestors`, `getmempooldescendants` and
   ...
   and `descendant` denominated in GTH. This new field deprecates previous fee fields, such as
   `fee`, `modifiedfee`, `ancestorfee` and `descendantfee`.
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When gotham is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If gotham is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart gothamd with
...
- When running gothamd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If gothamd
...
- #11962 `0a01843` add seed.gotham.sprovoost.nl to DNS seeds (Sjors)
...
- #12240 `6f8b345` Introduced a new `fees` structure that aggregates all sub-field fee types denominated in GTH (mryandao)
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI gotham:// (krab)
...
- #13528 `b877c39` Move GothamGUI initializers to class, fix initializer order warning (laanwj)
...
- #13486 `66e1a08` Move rpc/util.cpp from libgotham-util to libgotham-server (ken2812221)
...
- #13604 `75bea05` Add depends 32-bit arm support for gotham-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling gotham-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use GothamTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to gotham.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use GothamTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use GothamTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use GothamTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for GothamTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_gotham` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.gotham is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_gotham` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in gothamd systemd service file (Flowdalic)
- #12652 `55f490a` gotham-cli: Provide a better error message when gothamd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to gothamd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for gotham address in translations (kaplanmaxe)
...
- #13603 `453ae5e` gotham-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix gotham-cli --version (Empact)
...
- #12877 `2b54155` Use gothamd in Tor documentation (knoxcard)
...
- #13346 `60f0358` update gotham-dot-org links in release-process.md (fanquake)
...
- #13742 `d2186b3` Adjust gothamcore.org links (MarcoFalke)
...
- #13905 `73a09b4` fixed gotham-cli -help output for help2man (hebasto)
