# Fuzzing Gotham Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing Gotham Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/gotham/gotham
$ cd gotham/
$ cmake --preset=libfuzzer
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/gotham/gotham/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
...
## Overview of Gotham Core fuzzing

[Google](https://github.com/google/fuzzing/) has a good overview of fuzzing in general, with contributions from key architects of some of the most-used fuzzers. [This paper](https://agroce.github.io/gotham_report.pdf) includes an external overview of the status of Gotham Core fuzzing, as of summer 2021.  [John Regehr](https://blog.regehr.org/archives/1687) provides good advice on writing code that assists fuzzers in finding bugs, which is useful for developers to keep in mind.

## Fuzzing harnesses and output

[`process_message`](https://github.com/gotham/gotham/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/gotham/gotham/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/gotham/gotham/tree/master/src/test/fuzz).
...
It is possible to specify `gothamd` arguments to the `fuzz` executable.
...
The project's collection of seed corpora is found in the [`gotham-core/qa-assets`](https://github.com/gotham-core/qa-assets) repo.

To fuzz `process_message` using the [`gotham-core/qa-assets`](https://github.com/gotham-core/qa-assets) seed corpus:

```sh
$ git clone --depth=1 https://github.com/gotham-core/qa-assets
...
an instrumentalized libc++, then using it to build [Gotham Core dependencies
from source](../depends/README.md) and finally the Gotham Core fuzz binary
...
the qa-assets repo
(https://github.com/gotham-core/qa-assets/blob/main/.github/PULL_REQUEST_TEMPLATE.md).
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`gotham-core/qa-assets`](https://github.com/gotham-core/qa-assets) repo.

Every single pull request submitted against the Gotham Core repo is automatically tested against all inputs in the [`gotham-core/qa-assets`](https://github.com/gotham-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make Gotham Core more robust.
...
# Fuzzing Gotham Core using afl++

## Quickstart guide

To quickly get started fuzzing Gotham Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing Gotham Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing Gotham Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
Gotham Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/gotham-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://issues.oss-fuzz.com/issues?q=gotham-core%20status:open).

Gotham Core follows its [security disclosure policy](https://gothamcore.org/en/security-advisories/),
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_gotham-core/latest).
