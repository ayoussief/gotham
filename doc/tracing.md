# User-space, Statically Defined Tracing (USDT) for Gotham Core

Gotham Core includes statically defined tracepoints to allow for more
...
**Note**: Gotham Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Gotham Core
...
Multiple tools can list the available tracepoints in a `gothamd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Gotham Core, use `info probes` in `gdb`:

```
$ gdb ./build/bin/gothamd
…
(gdb) info probes
Type Provider   Name             Where              Semaphore Object
stap net        inbound_message  0x000000000014419e 0x0000000000d29bd2 /build/bin/gothamd
stap net        outbound_message 0x0000000000107c05 0x0000000000d29bd0 /build/bin/gothamd
stap validation block_connected  0x00000000002fb10c 0x0000000000d29bd8 /build/bin/gothamd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Gotham Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./build/bin/gothamd | grep NT_STAPSDT -A 4 -B 2
