# CJDNS support in Gotham Core

It is possible to run Gotham Core over CJDNS, an encrypted IPv6 network that
...
done outside of Gotham Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Gotham network and individual nodes.
...
before it will work with your Gotham Core node. This is described in steps
...
## Run Gotham Core with CJDNS

Once you are connected to the CJDNS network, the following Gotham Core
...
When enabled, this option tells Gotham Core that it is running in an
...
IPv6 local network. This helps Gotham Core perform better address management:
...
CJDNS support was added to Gotham Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `gotham-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/gotham/gotham/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Gotham Core

There are several ways to see your CJDNS address in Gotham Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `gotham-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `gotham-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `gotham-cli help getnodeaddresses` for details.
