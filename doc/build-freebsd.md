...
This guide describes how to build gothamd, command-line utilities, and GUI on FreeBSD.
...
### 2. Clone Gotham Repo
Now that `git` and all the required dependencies are installed, let's clone the Gotham Core repository to a directory. All build scripts and commands will run from this directory.
```bash
...
#### Wallet Dependencies
It is not necessary to build wallet functionality to run either `gothamd` or `gotham-qt`.
