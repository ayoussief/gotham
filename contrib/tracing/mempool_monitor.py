...
""" Example logging Gotham Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into gothamd with pid {pid}")
    gothamd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    gothamd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    gothamd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    gothamd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    gothamd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[gothamd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of gothamd>")
        exit(1)
