...
Tracing (USDT) support for Gotham Core on Linux using. For more information on
USDT support in Gotham Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `gothamd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `gothamd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[Gotham Core USDT documentation] on how to list available tracepoints in your
`gothamd` binary.

[Gotham Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py $(pidof gothamd)
```
...
Gotham P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py $(pidof gothamd)
```
...
In a different terminal, starting Gotham Core in SigNet mode and with
re-indexing enabled.

```
$ ./build/bin/gothamd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py $(pidof gothamd)
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py $(pidof gothamd)
```
