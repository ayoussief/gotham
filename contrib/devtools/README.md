...
Provides utilities for managing copyright headers of `The Gotham Core
...
Generates a gotham.conf file in `share/examples/` by parsing the output from `gothamd --help`. This script is run during the
release process to include a gotham.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-gotham-conf.sh
```
