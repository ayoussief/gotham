#!/usr/bin/env bash
# Copyright (c) 2019-2022 The Gotham Core developers
...
(
    ...
    case "$HOST" in
        *mingw*)
            ...
            mv build/gotham-win64-setup.exe "${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our Gotham Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built Gotham Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example gotham.conf file. if contrib/devtools/gen-gotham-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/gotham.conf" "${DISTNAME}/"
    ...
    case "$HOST" in
        ...
        *darwin*)
            ...
            mv build/dist/Gotham-Core.zip "${OUTDIR}/${DISTNAME}-${HOST}-unsigned.zip"
