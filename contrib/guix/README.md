# Bootstrappable Gotham Core Builds

This directory contains the files necessary to perform bootstrappable Gotham
Core builds.
...
To build Gotham Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Gotham Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/gotham-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/gotham-core/guix.sigs).
