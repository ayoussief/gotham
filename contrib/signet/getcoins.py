#!/usr/bin/env python3
# Copyright (c) 2020-2022 The Gotham Core developers
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing gotham-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='gotham-cli', help='gotham-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='Gotham address to which the faucet should send')
...
parser.add_argument('gotham_cli_args', nargs='*', help='Arguments to pass on to gotham-cli (default: -signet)')

args = parser.parse_args()

if args.gotham_cli_args == []:
    args.gotham_cli_args = ['-signet']


def gotham_cli(rpc_command_and_params):
    argv = [args.cmd] + args.gotham_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = gotham_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = gotham_cli(['getnewaddress', 'faucet', 'bech32'])
