...
"""Script for verifying Gotham Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from gothamcore.org and gotham.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/gotham-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://gothamcore.org"
HOST2 = "https://gotham.org"
VERSIONPREFIX = "gotham-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Gotham community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"gotham_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by gothamcore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the gotham release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes gotham.org lags behind gothamcore.org.)')
    )
